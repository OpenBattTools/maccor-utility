...
from helperfunctions import print_
...
class TDLLScopeTrace(ctypes.Structure):
    ...
    _pack_ = 1  # packed record
    _fields_ = [
        ('Samples', ctypes.c_byte),
        ('Reading', TScopeTraceVI * 50)
    ]
    field_strings_ = [field_tpl[0] for field_tpl in _fields_]
...
class MacDataFile:
    ...
    def read(self, debug: bool = False) -> dict:
        ...
        # stdcall
        dll_path = str(
            Path(__file__).parent / 'maccor_dll' / 'MacReadDataFileLIB64bit.dll'
        )
        ...
        try:
            ...
            if file_handle >= 0:
                ...
                while dll.LoadAndGetNextTimeData(
                        file_handle,
                        ctypes.pointer(dll_time_data)
                ) == 0:
                    try:
                        sub_list = []
                        # Time series data
                        ...
                        for can_num in range(0, 2):  # 0, 1
                            dll.GetCANData(
                                file_handle,
                                can_num,
                                ctypes.pointer(CAN_val)
                            )
                            can_str += ('\nThermistor ' + str(can_num) + ': ' +
                                        "%.3f" % CAN_val.value)
...
if __name__ == '__main__':
    ...
    path2file = r'test_data/231004_test_data_raw.024'
